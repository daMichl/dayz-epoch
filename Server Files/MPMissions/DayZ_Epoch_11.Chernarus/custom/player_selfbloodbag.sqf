...
if (dayz_combat == 1) then { // Check if in combat
    cutText [format["You can´t use a bloodbag in combat!"], "PLAIN DOWN"]; //display text at bottom center of screen when in combat
} else {
	...
	if (_finished) then {
		...
		if (random(_infectionChance) < 1) then {
			...
			cutText [format["The bloodbag you used was infected! Use antibiotics!"], "PLAIN DOWN"]; //display text at bottom center of screen if infected
			...
			cutText [format["The bloodtransfusion was succesful!"], "PLAIN DOWN"]; //display text at bottom center of screen on succesful self bloodbag
		};
		...
		cutText [format["You have interrupted the bloodtransfusion!"], "PLAIN DOWN"]; //display text at bottom center of screen on interrupt
	};
};